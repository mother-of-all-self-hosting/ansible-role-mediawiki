...
#
# Tags look like `v<MediaWiki version>-<release>`:
#
# - if defaults/main.yml points at a MediaWiki version that has never been
#   released, the release counter restarts at 0 (`v1.43.9-0`)
# - otherwise the counter is incremented (`v1.43.9-1`), but only if something
...
# Anchored on `mediawiki_version:` so that neither the similarly named
# `mediawiki_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^mediawiki_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the MediaWiki version from $defaults_path"
	exit 1
fi

# The tags of this repository carry a leading `v` (`v1.43.3-0`) while the
# version does not (`1.43.9`), and a version which does carry one must not end
# up with two.
